...
```nsys analyze``` 命令会基于现有的分析结果，利用专家系统规则生成报告，并将报告输出到终端。这些报告是根据 .nsys-rep 文件导出的 SQLite 数据库生成的。如果指定了一个 .nsys-rep 文件，Nsight Systems 会查找其对应的 SQLite 文件并使用它；
如果尚不存在 SQLite 导出文件，则会自动创建一个。

选择“analyze”命令开关后，将提供以下选项。用法：
...
| 选项 | 可选参数（默认值加粗） | 说明 |
|-----|-----------------------|-------|
| --help | <tag>, none | 打印帮助信息。该选项可接受一个可选参数作为“标签”（tag）。如果提供了标签，则仅打印与该标签相关的选项。 |
| --format 或 -f | column, table, csv, tsv, json, hdoc, htable,. | 指定输出格式。特殊名称 “.” 表示使用给定输出的默认格式：</br> 对于控制台（console）输出，默认格式为 column；</br>对于文件（files）和进程输出（process outputs），默认格式为 csv</br>此选项可多次使用，也可通过逗号分隔的列表一次性指定多种格式，格式为：<name[:args…]>（即格式名称后可选地跟冒号和参数）|
| --force-export | true, **false** | 强制从指定的报告文件重新导出 SQLite 文件，即使该 SQLite 文件已存在 |
| --force-overwrite | true, **false** | 覆盖任何已存在的输出文件。 |
| --help-formats | <format_name>, ALL, **[none]** | 不带参数时，列出所有可用输出格式的简要摘要。如果指定了某个格式名称，则显示该格式的详细说明。如果指定为 ALL，则显示所有可用格式的详细说明。 |
| --help-rules | <rule_name>, ALL, **[none]** | 
| --output 或 -o | -, @<command>, <basename>, . | 指定输出机制。共有三种输出方式：</br>打印到控制台（console）</br>输出到文件（file）</br>输出到外部命令（command）</br>该选项可多次使用，也可通过逗号分隔的列表一次性指定多个输出目标。</br>具体规则如下：</br>如果输出名称为 "-"，则结果将显示在控制台上。</br>如果输出名称以 "@" 开头，则表示要执行一个外部命令。nsys 会运行该命令，并将分析结果通过管道（pipe）传入该命令的标准输入（stdin）。该命令的标准输出（stdout）和标准错误（stderr）仍连接到控制台，因此其输出会直接显示在终端上。</br>其他任何输出名称均被视为文件的基本路径和名称（basename）。实际生成的文件名格式为：</br><basename>_<analysis&args>.<output_format></br>默认的 basename（含路径）是根据输入的 .nsys-rep 文件或 --sqlite 选项推导出的 SQLite 文件名（不含扩展名）。</br>使用 "." 作为输出名称，表示应将分析结果输出到文件，并使用默认的 basename。</br>若要使用默认 basename 将一个或多个分析结果写入文件，请使用 --output 选项。</br>⚠️ 关于命令输出（@command）的重要限制：</br>不支持 shell 扩展：如 *、?、[]、~ 等通配符或路径扩展均无效。</br>不支持 shell 重定向或管道：不能在命令字符串中使用 |、>、>> 等 shell 语法。</br>命令解析方式简单：命令及其参数通过空白字符分割，不支持引号（例如无法处理带空格的参数）。</br>建议：对于需要复杂命令行语法的场景，应将命令写入一个 shell 脚本文件，然后将该脚本指定为输出命令（例如 @./my_script.sh）。</br>此机制允许灵活地将分析结果直接送入其他工具（如 grep、jq、自定义解析器等），但需注意上述解析限制。</br> |
| --quite 或 -q | | 不显示详细信息，仅显示错误消息。 |
| --rule 或 -r | cuda_memcpy_async, cuda_memcpy_sync, cuda_memset_sync, cuda_api_sync, gpu_gaps, gpu_time_util, dx12_mem_ops, **all** | 指定要执行的规则（rule）及其可选参数。该选项可多次使用，也可通过逗号分隔的列表一次性指定多个规则。 |
| --sqlite | <file.sqlite> | 指定 SQLite 导出文件的名称。</br>如果该文件已存在，则直接使用它；</br>如果该文件不存在，或者使用了 --force-export 选项，则会先从指定的 .nsys-rep 文件生成此 SQLite 文件，然后再进行后续处理。</br>⚠️ 注意：当输入文件本身就是一个 SQLite 文件时，不能使用此选项。 |
| --timeunit | nsec, usec, msec, **nanoseconds**, microseconds, milliseconds, seconds | 设置时间的基本单位。该选项的参数通过最长前缀匹配方式进行识别，这意味着无需输入完整单词，只需提供足够区分的前缀即可（例如 m 可匹配 millisecond，u 可匹配 microsecond）。</br>此选项的作用类似于为每个格式化器（formatter）隐式传递一个 :time=<unit> 参数，但格式化器本身使用更严格的命名规范。 |
