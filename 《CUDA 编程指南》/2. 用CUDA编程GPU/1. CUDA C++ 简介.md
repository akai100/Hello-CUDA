...
使用统一内存时，有一些CUDA API（将在[内存建议和预取](https://github.com/akai100/Hello-CUDA/blob/main/%E3%80%8ACUDA%20%E7%BC%96%E7%A8%8B%E6%8C%87%E5%8D%97%E3%80%8B/2.%20%E7%94%A8CUDA%E7%BC%96%E7%A8%8BGPU/4.%20%E7%BB%9F%E4%B8%80%E5%86%85%E5%AD%98%E4%B8%8E%E7%B3%BB%E7%BB%9F%E5%86%85%E5%AD%98.md)中介绍），它们会向管理内存的NVIDIA驱动程序提供提示，这能够在使用统一内存时发挥显式内存管理的部分性能优势。

## 4. 同步CPU和GPU

正如启动内核中所提到的，内核启动相对于调用它们的CPU线程是异步的。这意味着CPU线程的控制流会在内核完成之前继续执行，甚至可能在内核启动之前就继续执行。为了确保内核在主机代码继续执行之前已完成运行，需要某种同步机制。

同步GPU和主机线程的最简单方法是使用```cudaDeviceSynchronize```，它会阻塞主机线程，直到GPU上所有先前发出的任务都完成。在本章的示例中，这已经足够了，因为GPU上只执行单个操作。在更大的应用程序中，可能会有多个流在GPU上执行任务，
而```cudaDeviceSynchronize```会等待所有流中的任务完成。在这些应用程序中，建议使用流同步API仅与特定流同步，或者使用CUDA事件。这些内容将在异步执行章节中详细介绍。

## 5. 综合运用

## 6. 运行时初始化

CUDA运行时会为系统中的每个设备创建一个CUDA上下文。此上下文是该设备的主上下文，会在第一个需要该设备上有活动上下文的运行时函数被调用时初始化。

该上下文由应用程序的所有主机线程共享。作为上下文创建的一部分，设备代码会在必要时进行即时编译，并加载到设备内存中。所有这些操作都是透明进行的。如运行时API与驱动程序API之间的互操作性中所述，CUDA运行时创建的主上下文可从驱动程序API进行访问以实现互操作性。

从CUDA 12.0开始，```cudaInitDevice```和```cudaSetDevice```调用会初始化运行时以及与指定设备相关联的主context。如果在这些调用之前出现运行时API请求，运行时将根据需要隐式使用设备0并自行初始化以处理这些请求。
这在对运行时函数调用进行计时以及解释首次调用运行时所返回的错误代码时非常重要。在CUDA 12.0之前，```cudaSetDevice```不会初始化运行时。

```cudaDeviceReset``` 会销毁当前设备的主上下文。如果在主上下文被销毁后调用 CUDA 运行时 API，将会为该设备创建一个新的主上下文。

## 7. CUDA中的错误检查

每个CUDA API都会返回一个枚举类型的值，即```cudaError_t```。在示例代码中，这些错误通常不会被检查。在生产应用程序中，最佳实践是始终检查并管理每个CUDA API调用的返回值。当没有错误时，返回的值为```cudaSuccess```。许多应用程序选择实现一个实用宏，如下所示。
...
此宏使用```cudaGetErrorString``` API，该API返回一个人类可读的字符串，用于描述特定```cudaError_t```值的含义。使用上述宏时，应用程序会在```CUDA_CHECK(expression)```宏中调用CUDA运行时API，如下所示：
...
如果这些调用中有任何一个检测到错误，该错误将通过此宏输出到```stderr```。此宏在小型项目中很常见，但在大型应用程序中可以适配日志系统或其他错误处理机制。

### 7.1. 错误状态

CUDA运行时为每个主机线程维护一个```cudaError_t```状态。该值默认为```cudaSuccess```，且会在发生错误时被覆盖。```cudaGetLastError```会返回当前错误状态，然后将其重置为```cudaSuccess```。此外，```cudaPeekLastError```会返回错误状态但不会重置它。

使用三重尖括号表示法启动的内核不会返回```cudaError_t```。在启动内核后立即检查错误状态是一种良好的做法，这样可以检测内核启动时的即时错误或内核启动前的异步错误。

在内核启动后立即检查错误状态时得到```cudaSuccess```值，并不意味着内核已成功执行，甚至不意味着内核已开始执行。它仅能验证传递给运行时的内核启动参数和执行配置未触发任何错误，且错误状态不是内核启动前的先前错误或异步错误。

### 7.2 异步错误

CUDA内核启动和许多运行时API是异步的。异步CUDA运行时API将在异步执行中详细讨论。每当发生错误时，CUDA错误状态就会被设置并覆盖。

这意味着异步操作执行过程中发生的错误只有在下次检查错误状态时才会被报告。如前所述，这可能是对```cudaGetLastError```、```cudaPeekLastError```的调用，也可能是任何返回```cudaError_t```的CUDA API。

当CUDA运行时API函数返回错误时，错误状态不会被清除。这意味着，来自异步错误（例如内核的无效内存访问）的错误代码将由每个CUDA运行时API返回，直到通过调用```cudaGetLastError```清除错误状态为止。
...
识别CUDA错误的另一个好方法是使用```CUDA_LOG_FILE```环境变量。当设置了这个环境变量时，CUDA驱动程序会将遇到的错误消息写入到该环境变量中指定路径的文件里。例如，以下这段错误的CUDA代码尝试启动一个线程块，而该线程块的规模超过了任何架构所支持的最大值。
...
构建并运行此程序时，内核启动后的检查会使用2.1.7节中所示的宏来检测并报告错误。


然而，当应用程序在将```CUDA_LOG_FILE```设置为文本文件的情况下运行时，该文件会包含更多关于此错误的信息。
...
将```CUDA_LOG_FILE```设置为```stdout```或```stderr```，将分别输出到标准输出和标准错误。通过```CUDA_LOG_FILE```环境变量，即使应用程序未对CUDA返回值执行适当的错误检查，也可以捕获和识别CUDA错误。
这种方法在调试时可能非常有效，但仅靠环境变量无法让应用程序在运行时处理CUDA错误并从中恢复。CUDA的错误日志管理功能还允许向驱动程序注册一个回调函数，每当检测到错误时，该函数就会被调用。这可用于在运行时捕获和处理错误，还能将CUDA错误日志无缝集成到应用程序现有的日志系统中。

第4.8节展示了更多CUDA错误日志管理功能的示例。错误日志管理和```CUDA_LOG_FILE```在NVIDIA驱动程序r570及更高版本中可用。


## 8. 设备和主机函数

```__global__``` 说明符用于指示内核的入口点。也就是说，这是一个将在 GPU 上并行执行的函数。大多数情况下，内核是从主机启动的，但也可以使用 动态并行性 从另一个内核中启动内核。

说明符```__device__```表示函数应编译为适用于GPU，并且可以从其他```__device__```或```__global__```函数调用。包括类成员函数、仿函数和lambda在内的函数，可以同时指定为```__device__```和```__host__```.

## 9. 变量说明符

CUDA 说明符可用于静态变量声明以控制其存储位置。

+ ```__device__```指定变量存储在 全局内存 中

+ ```__constant__``` 指定变量存储在 常量内存 中

+ ```__managed__``` 指定变量以统一内存的形式存储

+ ```__shared__``` 表明变量存储在 共享内存 中

在```__device__```或```__global__```函数内声明且不带任何说明符的变量，在可能的情况下会被分配到寄存器中，必要时则会被分配到本地内存中。在```__device__```或```__global__```函数外声明且不带任何说明符的变量，将被分配到系统内存中。

### 9.1. 检测设备编译

当一个函数用```__host__``` ```__device__```指定时，编译器会被指示为该函数生成GPU和CPU代码。在这类函数中，可能希望使用预处理器来仅为函数的GPU版本或CPU版本指定代码。如下例所示，检查```__CUDA_ARCH_```是否已定义是实现此目的的最常见方法。

## 10. 线程快集群

从计算能力 9.0 及更高版本开始，CUDA 编程模型包含一个可选的层级结构，称为线程块集群，它由线程块组成。与线程块中的线程保证在流式多处理器上共同调度类似，集群中的线程块也保证在 GPU 的 GPU 处理集群（GPC）上共同调度。
